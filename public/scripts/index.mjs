...
import { filterEntitiesById, filterEntitiesByListOfIds, filterEntitiesByUserId, } from "./utils/find.mjs";
(function () {
    return __awaiter(this, void 0, void 0, function* () {
      ...
      `);
        ...
        <div class="posts__post" aria-labelledby="${post.id}" aria-ownedby="${post.user_id}">
          ...
          <a href="${baseURL}?postId=${post.id}" class="posts__post-title">${post.title}</a>
      ...
      `);
            ...
            const userPostCommented = filterEntitiesByListOfIds(posts, userComments.map(({ post_id }) => post_id));
            console.log(userComments.map(({ post_id }) => post_id), userPostCommented);
        ...
        commentsData.forEach((comment) => {
            ...
            const postComments = postsSection.querySelectorAll(`div.posts__post[aria-labelledby="${comment.post_id}"] .posts__post-comments`);
            if (!postComments.length)
                return;
            postComments.forEach((postComment) => postComment.insertAdjacentHTML("beforeend", `
        <div class="posts__comment" aria-labelledby="${comment.id}" aria-ownedby="${comment.user_id}">
          <a href="${baseURL}?userId=${comment.user_id}" class="posts__comment-user">
      ...
      `));
        ...
        users.forEach((user) => {
            ...
            const postUsers = postsSection.querySelectorAll(`div.posts__post[aria-ownedby="${user.id}"] .posts__post-user`);
            if (postUsers.length) {
                postUsers.forEach((postUser) => postUser.insertAdjacentHTML("afterbegin", `
      ...
      `));
            ...
            const commentUsers = postsSection.querySelectorAll(`div.posts__comment[aria-ownedby="${user.id}"] .posts__comment-user`);
            if (commentUsers) {
                commentUsers.forEach((commentUser) => commentUser.insertAdjacentHTML("afterbegin", `
